...
function parseTagResponse(body, tags) {
    const out = {};
    for (const tag of tags) out[tag] = null;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const tagName = headerFields[0].slice(1);
        if (!wanted.has(tagName)) continue;
        const status = headerFields[1] || '';
        if (status && status !== 'S_OK') continue;
        const valueLine = lines[i + 1];
        if (!valueLine) continue;
        const valueFields = valueLine.split('\t');
        if (valueFields.length < 2) continue;
        const raw = valueFields[valueFields.length - 1];
        const n = Number(raw);
        out[tagName] = Number.isFinite(n) ? n : raw;
    }
    return out;
}
